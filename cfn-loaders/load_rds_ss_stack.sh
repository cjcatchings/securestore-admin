...
if [ $(aws dynamodb list-tables --profile $AWS_PROFILE --query "contains(TableNames, 'SecStoreAndMoreAdmin')") == true ]; then
  ...
  aws dynamodb put-item \
    ...
    --profile $AWS_PROFILE \
    --item "{\"ResourceIdentifier\": {\"S\": \"$DBS_URI_RI\"}, \"Value\": {\"S\": \"$(aws cloudformation describe-stacks --stack-name Eks-Rds-SS-Stack-CF --query 'Stacks[0].Outputs[?OutputKey==`SecStoreRDSUrl`].OutputValue' --output text --profile $AWS_PROFILE)\"}}"
else
